...
(ert-deftest t--make-cache-oclosure ()
  ...
  (let ((x (t--make-cache-oclosure)))
    (should (equal (t--oinfo--cnt x) 0))
    (should (equal (t--oinfo--pid x) nil))
    (should (equal (t--oinfo--val x) nil)))
  (let ((info '(:a 1 :b 2 :c 3))
        (info2 '(:a 3 :b 2 :c 1))
        (oa (t--make-cache-oclosure))
        (ob (t--make-cache-oclosure))
        (oc (t--make-cache-oclosure)))
    ;; a
    (should (equal (funcall oa info :a) 1))
    (should (eq (t--oinfo--pid oa) info))
    (should (equal (t--oinfo--val oa) 1))
    (should (equal (t--oinfo--cnt oa) 1))
    (should (equal (funcall oa info :wtf) 1))
    ;; b
    (should (equal (funcall ob info :b) 2))
    (should (eq (t--oinfo--pid ob) info))
    (should (equal (t--oinfo--val ob) 2))
    (should (equal (t--oinfo--cnt ob) 1))
    (should (equal (funcall ob info :wtf) 2))
    ;; c
    (should (equal (funcall oc info :c) 3))
    (should (eq (t--oinfo--pid oc) info))
    (should (equal (t--oinfo--val oc) 3))
    (should (equal (t--oinfo--cnt oc) 1))
    (should (equal (funcall oc info :wtf) 3))
    ;; test cnt
    (funcall oa info :a)
    (should (equal (t--oinfo--cnt oa) 3))
    (funcall ob info :b) (funcall ob info :b)
    (should (equal (t--oinfo--cnt ob) 4))
    (funcall oc info :c) (funcall oc info :c) (funcall oc info :c)
    (should (equal (t--oinfo--cnt oc) 5))
    ;; change plist
    (should (equal (funcall oa info2 :a) 3))
    (should (eq (t--oinfo--pid oa) info2))
    (should (equal (funcall ob info2 :b) 2))
    (should (eq (t--oinfo--pid ob) info2))
    (should (equal (funcall oc info2 :a) 3))
    (should (eq (t--oinfo--pid oc) info2))))

(ert-deftest t--oinfo-pget ()
  "Tests for `org-w3ctr--pget'."
  (dlet ((t--oinfo-cache-props '(:a :b))
        (t--oinfo-cache-table (make-hash-table))
        (info '(:a 1 :b 2 :c 3)))
    (dolist (a t--oinfo-cache-props)
      (puthash a (t--make-cache-oclosure) t--oinfo-cache-table))
    (should (equal (eval '(t--pget info :a)) 1))
    (should (equal (eval '(t--oinfo--cnt (gethash :a t--oinfo-cache-table)))
                   1))
    (should (equal (eval '(t--pget info :b)) 2))
    (should (equal (eval '(t--oinfo--cnt (gethash :b t--oinfo-cache-table)))
                   1))
    (should (equal (eval '(t--pget info :c)) 3))
    (should-not (gethash :c t--oinfo-cache-table))))
...
(ert-deftest t--oinfo-pput ()
  ...
  (dlet ((t--oinfo-cache-props '(:a :b))
    ...
    (dolist (a t--oinfo-cache-props)
      (puthash a (t--make-cache-oclosure) t--oinfo-cache-table))
    ;; a
    (eval '(t--pput info :a 2))
    (should (equal (eval '(t--pget info :a)) 2))
    (should (equal (t--oinfo--val (gethash :a t--oinfo-cache-table)) 2))
    (should (equal (plist-get info :a) 1))
    ;; b
    (eval '(t--pput info :b 3))
    (should (equal (eval '(t--pget info :b)) 3))
    (should (equal (t--oinfo--val (gethash :b t--oinfo-cache-table)) 3))
    (should (equal (plist-get info :b) 2))
    ;; c
    (eval '(t--pput info :c 4))
...
(ert-deftest t--oinfo-cleanup ()
  ...
  (dlet ((t--oinfo-cache-props '(:a :b))
    ...
    (eval '(t--pget info :a))
    (eval '(t--pget info :b))
    (should (eq (t--oinfo--pid (gethash :a t--oinfo-cache-table)) info))
    (should (eq (t--oinfo--pid (gethash :b t--oinfo-cache-table)) info))
    (should (equal (t--oinfo--val (gethash :a t--oinfo-cache-table)) 1))
    (should (equal (t--oinfo--val (gethash :b t--oinfo-cache-table)) 2))
    (should (equal (t--oinfo--cnt (gethash :a t--oinfo-cache-table)) 1))
    (should (equal (t--oinfo--cnt (gethash :b t--oinfo-cache-table)) 1))
    (t--oinfo-cleanup)
    (should (eq (t--oinfo--pid (gethash :a t--oinfo-cache-table)) nil))
    (should (eq (t--oinfo--pid (gethash :b t--oinfo-cache-table)) nil))
    (should (equal (t--oinfo--val (gethash :a t--oinfo-cache-table)) nil))
    (should (equal (t--oinfo--val (gethash :b t--oinfo-cache-table)) nil))
    (should (equal (t--oinfo--cnt (gethash :a t--oinfo-cache-table)) 1))
    (should (equal (t--oinfo--cnt (gethash :b t--oinfo-cache-table)) 1))))

