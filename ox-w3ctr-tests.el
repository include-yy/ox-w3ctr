...
(ert-deftest t--timezone-to-offset ()
  ...
  ($l (t--timezone-to-offset "local") 'local)
  ($l (t--timezone-to-offset "LOCAL") 'local)
  ($l (t--timezone-to-offset "LoCaL") 'local)
  ($l (t--timezone-to-offset "lOcAl") 'local)
  ($l (t--timezone-to-offset "UTC+8") (* 8 3600))
  ($l (t--timezone-to-offset "UTC+08") 28800)
  ($l (t--timezone-to-offset "GMT-5") (* -5 3600))
  ($l (t--timezone-to-offset "+0530") (+ (* 5 3600) (* 30 60)))
  ($l (t--timezone-to-offset "-0830") (+ (* -8 3600) (* -30 60)))
  ($n (t--timezone-to-offset "+10"))
  ($n (t--timezone-to-offset "-11"))
  ($n (t--timezone-to-offset "INVALID"))
  ($n (t--timezone-to-offset "UTC+123"))
  ($n (t--timezone-to-offset "+12345"))
  ($n (t--timezone-to-offset "+1400"))
  ($n (t--timezone-to-offset "UTC+13"))
  ($n (t--timezone-to-offset "UTC-13"))
  ($n (t--timezone-to-offset "+0860")))

(ert-deftest t--get-info-timezone-offset ()
  "Tests for `org-w3ctr--get-info-timezone-offset'."
  (let ((info0 '(:html-timezone "local")))
    ($l (t--get-info-timezone-offset info0) 'local))
  (let ((info1 '(:html-timezone 28800)))
    ($l (t--get-info-timezone-offset info1) 28800))
  (let ((info2 '(:html-timezone -18000)))
    ($l (t--get-info-timezone-offset info2) -18000))
  (let ((info3 '(:html-timezone "UTC+8")))
    ($l (t--get-info-timezone-offset info3) 28800)
    ($y (numberp (t--pget info3 :html-timezone)))
    ($l (t--pget info3 :html-timezone) 28800))
  (let ((info4 '(:html-timezone "-0500")))
    ($l (t--get-info-timezone-offset info4) -18000)
    ($y (numberp (t--pget info4 :html-timezone)))
    ($l (t--pget info4 :html-timezone) -18000))
  (let ((info5 '(:html-timezone "UTC+0")))
    ($l (t--get-info-timezone-offset info5) 0)
    ($l (t--pget info5 :html-timezone) 0))
  (let ((info6 '(:html-timezone "+0530")))
    ($l (t--get-info-timezone-offset info6) 19800)
    ($l (t--pget info6 :html-timezone) 19800))
  (let ((info7 '(:html-timezone "Invalid")))
    ($e! (t--get-info-timezone-offset info7)))
  (let ((info8 '(:html-timezone 3600 :other "value")))
    ($l (t--get-info-timezone-offset info8) 3600)
...
(ert-deftest t--get-info-timezone-delta ()
  ...
  (let ((info '(:html-timezone 2 :html-export-timezone 1)))
    ($l (t--get-info-timezone-delta info) -1))
  (let ((info '(:html-timezone 28800 :html-export-timezone 0)))
    ($l (t--get-info-timezone-delta info) -28800))
  (let ((info '(:html-timezone local :html-export-timezone 3600)))
    ($l (t--get-info-timezone-delta info) 0))
  (let ((info '(:html-timezone local :html-export-timezone 3600)))
    ($l (t--get-info-timezone-delta info) 0))
  (let ((info '(:html-timezone 114514 :html-export-timezone 191981)))
    ($l (t--get-info-timezone-delta info) 77467))
  (let ((info '(:html-timezone "WTF" :html-export-timezone "INVALID")))
    ($e! (t--get-info-timezone-delta info)))
  (let ((info '(:html-timezone 0 :html-export-timezone "INVALID")))
    ($e! (t--get-info-timezone-delta info)))
  ($e! (t--get-info-timezone-delta nil))
  (let ((info '(:html-timezone 114514 :html-export-timezone 191981)))
    ($l (t--get-info-timezone-delta info 1 2) 1))
  (let ((info '(:html-timezone 114514 :html-export-timezone 191981)))
    ($l (t--get-info-timezone-delta info nil 114515) 1))
  (let ((info '(:html-timezone 114514 :html-export-timezone 191981)))
    ($l (t--get-info-timezone-delta info 191980) 1))
  (let ((info '(:html-timezone 114514 :html-export-timezone 191981)))
    ($l (t--get-info-timezone-delta nil 114514 191981) 77467)))

(ert-deftest t--get-datetime-format ()
  "Tests for `org-w3ctr--get-datetime-format'."
  ($l (t--get-datetime-format 28800 'space-none) "%F %R+0800")
  ($l (t--get-datetime-format 18000 'space-none) "%F %R+0500")
  ($l (t--get-datetime-format -28800 'space-none) "%F %R-0800")
  ($l (t--get-datetime-format -18000 'space-none) "%F %R-0500")
  ($l (t--get-datetime-format 0 'space-none) "%F %R+0000")
  ($l (t--get-datetime-format 0 'space-none-zulu) "%F %RZ")
  ($l (t--get-datetime-format 0 'space-colon) "%F %R+00:00")
  ($l (t--get-datetime-format 0 'space-colon-zulu) "%F %RZ")
  ($l (t--get-datetime-format 4800 'space-colon-zulu) "%F %R+01:20")
  ($l (t--get-datetime-format 0 'T-colon) "%FT%R+00:00")
  ($l (t--get-datetime-format 0 'T-none) "%FT%R+0000")
  ($l (t--get-datetime-format 0 'T-colon) "%FT%R+00:00")
  ($l (t--get-datetime-format 0 'T-colon-zulu) "%FT%RZ")
  ($l (t--get-datetime-format 3600 'space-colon) "%F %R+01:00")
  ($l (t--get-datetime-format -900 'space-colon) "%F %R-00:15")
  ($l (t--get-datetime-format 19800 'T-colon) "%FT%R+05:30")
  ($l (t--get-datetime-format -16200 'T-colon) "%FT%R-04:30")
  ($l (t--get-datetime-format 50400 'space-none) "%F %R+1400")
  ($l (t--get-datetime-format -43200 'space-none) "%F %R-1200")
  ($l (t--get-datetime-format 37800 'T-colon-zulu) "%FT%R+10:30")
  ($n (t--get-datetime-format 0 nil))
  ($l (t--get-datetime-format 'local nil t) "%F")
  ($l (t--get-datetime-format 'local 'space-none) "%F %R")
  ($l (t--get-datetime-format 'local 'space-none-zulu) "%F %R")
  ($l (t--get-datetime-format 'local 'space-colon) "%F %R")
  ($l (t--get-datetime-format 'local 'space-colon-zulu) "%F %R")
  ($l (t--get-datetime-format 'local 'T-none) "%FT%R")
  ($l (t--get-datetime-format 'local 'T-none-zulu) "%FT%R")
  ($l (t--get-datetime-format 'local 'T-colon) "%FT%R")
  ($l (t--get-datetime-format 'local 'T-colon-zulu) "%FT%R"))
...
(ert-deftest t--has-math-p ()
  ...
  (cl-flet ((mkinfo (str) `( :with-latex t
    ...
    ($y (t--has-math-p (mkinfo "$1+2$")))
    ($y (t--has-math-p (mkinfo "\\(1+2\\)")))
    ($y (t--has-math-p (mkinfo "\\[1+2\\]")))
    ($y (t--has-math-p (mkinfo "\\begin_equation\n123\n\\end_equation")))))

(ert-deftest t--use-default-style-p ()
  "Tests for `org-w3ctr--use-default-styple-p'."
  ($n (t--use-default-style-p nil))
  ($y (t--use-default-style-p '(:html-head-include-default-style t))))
...
(ert-deftest t--format-home/up-list ()
  ...
  ($y (t--format-home/up-list nil nil))
